...
<div class="panel panel-default text-center">
                            <div class="panel-body" style="background-color:#d3c1ff;">
                                ...
                                <h4 style="color:blue;"><b>Registered Garajs in Auto Assist Company </b></h4>
                            </div>
                        </div>
        <div class="container">
 <a href="admin.php"><button type="button" class="btn btn-primary btn-arrow-left"><b>BACK</b></button></a>
        ...
        // $result = mysqli_query($conn, "SELECT * FROM `detail` INNER JOIN user on detail.nic=user.user_id WHERE (active=1 AND detail.nic in (SELECT nic FROM section WHERE section.section='Garaj')) OR (active=1 AND detail.nic in (SELECT nic FROM section WHERE section.section='Garaj'");
               ...
               // $result = mysqli_query($conn, "SELECT * FROM `detail` INNER JOIN user on detail.nic=user.user_id WHERE active=1 AND detail.nic in (SELECT nic FROM section WHERE section.section='Garaj')");
                $result = mysqli_query($conn, "SELECT * FROM `detail` INNER JOIN section on detail.nic=section.nic WHERE acceptorreject='1' AND section.section='Garaj'");
               
                    while ($row = mysqli_fetch_array($result)){ 
                    echo' <tr class="info">';
                    echo " <form action=\"garaj_detailsupdate.php\" method=\"post\">";
